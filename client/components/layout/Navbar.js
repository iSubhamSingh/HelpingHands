  ...
  const NavItem = ({ item }) => {
    ...
    return (
      <li className="relative group">
        ...
        <>
          <div onClick={() => { router.replace(item.link); navVisibilityHandler(); }} className="px-4 py-2 lg:group-hover:text-blue-700 group-hover:text-blue-950 cursor-pointer">
            {item.name}
          </div>
    ...
    <>
      <section>
        <header className="container min-w-full bg-blue-50 px-5 flex justify-between py-4 items-center fixed z-50 top-0 left-0 right-0">
          <div className="flex">
            <img className="w-10" src="/assets/images/icon/logo.svg" alt="logo" /><span className='font-bold text-4xl mx-4 text-blue-950'>HelpingHands</span>
          </div>
